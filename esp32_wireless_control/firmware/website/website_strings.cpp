// website_strings.cpp
#include "website_strings.h"

const char* SPEED = "speed";
const char* TRACKING_SPEED = "trackingSpeed";
const char* DIRECTION = "direction";
const char* SLEWING = "Slewing";
const char* PRESET = "preset";
const char* MODE = "mode";
const char* CAPTURE_MODE = "captureMode";
const char* EXPOSURE_TIME = "exposureTime";
const char* EXPOSURES = "exposures";
const char* PREDELAY = "preDelay";
const char* DELAY = "delay";
const char* FRAMES = "frames";
const char* PAN_ANGLE = "panAngle";
const char* PAN_DIRECTION = "panDirection";
const char* CONTINUOUS_PAN = "continuousPan";
const char* ENABLE_TRACKING = "enableTracking";
const char* DITHER_CHOICE = "ditherChoice";
const char* DITHER_FREQUENCY = "ditherFrequency";
const char* FOCAL_LENGTH = "focalLength";
const char* PIXEL_SIZE = "pixelSize";
const char* TRACKING_TYPE = "trackingType";
const char* CUSTOM_RATE = "customRate";
const char* DISABLE_TRACKING_ON_FINISH = "disableTrackingOnFinish";
const char* MIME_TYPE_TEXT = "text/plain";
const char* MIME_TYPE_HTML = "text/html";
const char* MIME_APPLICATION_JSON = "application/json";
const char* GOTO_RA = "gotoRA";
const char* STAR_CATALOG = "starCatalog";
const char* STAR_NAME = "starName";
