...
BSC5Notes::BSC5Notes(const uint8_t* start, const uint8_t* end) : _start(start), _end(end)
{
}
...
std::list<Note> BSC5Notes::search(const String query)
{
    ...
    while (!stop && current_pos < (const char*) _end)
    {
        ...
        //		if((n = unishox2_compress_simple(description, strlen(description), output_buf)) ==
        //-1) { 			print_out_nonl("error compressing: %d\n", n); 		} else { 			maxstrlen =
        //(strlen(description) > maxstrlen) ? strlen(description) : maxstrlen; 			total +=
        //strlen(description); 			maxstrlen2 = (strlen(output_buf) > maxstrlen2) ? strlen(output_buf) :
        //maxstrlen2; 			total2 += strlen(output_buf);
        //		}
        //		print_out_nonl("%d %s: %ld: %ld: %s\n", id, flags, strlen(description),
        //strlen(output_buf), description); 		print_out_nonl("%d %s: %s\n", id, flags, description);
        //		vTaskDelay(1);
    }
