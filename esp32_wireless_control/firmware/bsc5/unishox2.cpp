...
const char* USX_TEMPLATES[] = {"tfff-of-tfTtf:rf:rf.fffZ", "tfff-of-tf", "(fff) fff-ffff",
                               "tf:rf:rf", 0};
    ...
    switch (hcode)
    {
        case USX_ALPHA:
            if (*state != USX_ALPHA)
            {
                ...
                SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA],
                   ...
                   const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[])
{
    ...
    {
        SAFE_APPEND_BITS(*ol = append_switch_code(out, olen, *ol, *state));
        SAFE_APPEND_BITS(*ol = append_bits(out, olen, *ol, usx_hcodes[USX_DICT],
                                           usx_hcode_lens[USX_DICT]));
        // printf("Len:%d / Dist:%d/%.*s\n", longest_len, longest_dist, longest_len + NICE_LEN, in +
        // l - longest_dist - NICE_LEN + 1);
...
/// much of an issue.
int matchLine(const char* in, int len, int l, char* out, int olen, int* ol,
              struct us_lnk_lst* prev_lines, uint8_t* state, const uint8_t usx_hcodes[],
                      ...
                      const uint8_t usx_hcode_lens[])
{
    if (usx_hcode_lens[USX_ALPHA])
    {
        ...
        SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_vcodes[TERM_CODE & 0x1F],
                            ...
                            struct us_lnk_lst* prev_lines)
{
    ...
    state = USX_ALPHA;
    is_all_upper = 0;
    SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, UNISHOX_MAGIC_BITS,
                                                UNISHOX_MAGIC_BIT_LEN)); // magic bit(s)
    for (l = 0; l < len; l++)
    {
        if (usx_hcode_lens[USX_DICT] && l < (len - NICE_LEN + 1))
        {
            if (prev_lines)
            {
                l = matchLine(in, len, l, out, olen, &ol, prev_lines, &state, usx_hcodes,
                              usx_hcode_lens);
                if (l > 0)
                {
                    continue;
            ...
            if (in[l + 8] == '-' && in[l + 13] == '-' && in[l + 18] == '-' && in[l + 23] == '-')
            {
                ...
                if (uid_pos == l + 36)
                {
                    SAFE_APPEND_BITS2(rawolen,
                                      ol = append_nibble_escape(out, olen, ol, state, usx_hcodes,
                                                                usx_hcode_lens));
                    SAFE_APPEND_BITS2(
                        rawolen, ol = append_bits(out, olen, ol,
                    ...
                    for (uid_pos = l; uid_pos < l + 36; uid_pos++)
                    {
                        ...
                        if (c_uid != '-')
                            SAFE_APPEND_BITS2(
                                rawolen, ol = append_bits(out, olen, ol, getBaseCode(c_uid), 4));
                    }
                    // printf("GUID:\n");
                    l += 35;
                    continue;
        ...
        if (l < (len - 5) && usx_hcode_lens[USX_NUM])
        {
            ...
            if ((hex_type == USX_NIB_HEX_LOWER || hex_type == USX_NIB_HEX_UPPER) && hex_len > 3)
            {
                ...
                {
                    SAFE_APPEND_BITS2(rawolen,
                                      ol = append_bits(out, olen, ol, getBaseCode(in[l++]), 4));
                } while (--hex_len);
                l--;
                continue;
            }
            ...
            for (i = 0; i < 5; i++)
            {
                if (usx_templates[i])
                {
                    ...
                    if (((float) j / rem) > 0.66)
                    {
                        ...
                        rem = rem - j;
                        SAFE_APPEND_BITS2(rawolen,
                                          ol = append_nibble_escape(out, olen, ol, state,
                                                                    usx_hcodes, usx_hcode_lens));
                        SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, 0, 1));
                        SAFE_APPEND_BITS2(rawolen,
                                          ol = append_bits(out, olen, ol, (count_codes[i] & 0xF8),
                                                           count_codes[i] & 0x07));
        ...
        if (usx_freq_seq != NULL)
        {
            ...
            for (i = 0; i < 6; i++)
            {
                ...
                if (len - seq_len >= 0 && l <= len - seq_len)
                {
                    ...
                    {
                        SAFE_APPEND_BITS2(rawolen,
                                          ol = append_code(out, olen, ol, usx_freq_codes[i], &state,
                                                           usx_hcodes, usx_hcode_lens));
                        l += seq_len;
                        l--;
                        break;
        ...
        else
        {
            if (is_all_upper)
            {
                ...
                SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA],
                                                            usx_hcode_lens[USX_ALPHA]));
                state = USX_ALPHA;
            }
        }
        ...
        if (c_in >= 32 && c_in <= 126)
        {
            if (is_upper && !is_all_upper)
            {
                ...
                if (ll == l - 1)
                {
                    SAFE_APPEND_BITS2(rawolen, ol = append_switch_code(out, olen, ol, state));
                    SAFE_APPEND_BITS2(rawolen,
                                      ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA],
                                                       usx_hcode_lens[USX_ALPHA]));
                    state = USX_ALPHA;
                    is_all_upper = 1;
                }
            ...
            {
                uint8_t spl_code =
                    (c_in == ',' ? 0xC0 : (c_in == '.' ? 0xE0 : (c_in == ' ' ? 0 : 0xFF)));
                if (spl_code != 0xFF)
                {
                    uint8_t spl_code_len =
                        (c_in == ',' ? 3 : (c_in == '.' ? 4 : (c_in == ' ' ? 1 : 4)));
                    SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, UNI_STATE_SPL_CODE,
                                                                UNI_STATE_SPL_CODE_LEN));
                    SAFE_APPEND_BITS2(rawolen,
                                      ol = append_bits(out, olen, ol, spl_code, spl_code_len));
                    continue;
                }
            }
            c_in -= 32;
            ...
            if (c_in == 0)
            {
                if (state == USX_NUM)
                    SAFE_APPEND_BITS2(
                        rawolen, ol = append_bits(out, olen, ol, usx_vcodes[NUM_SPC_CODE & 0x1F],
                                                  usx_vcode_lens[NUM_SPC_CODE & 0x1F]));
                else
                    SAFE_APPEND_BITS2(
                        rawolen, ol = append_bits(out, olen, ol, usx_vcodes[1], usx_vcode_lens[1]));
            }
            else
            {
                c_in--;
                SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, usx_code_94[(int) c_in],
                                                            &state, usx_hcodes, usx_hcode_lens));
            }
        }
        ...
        {
            SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, CR_CODE, &state, usx_hcodes,
                                                        usx_hcode_lens));
        }
        else if (c_in == '\t')
        {
            SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, TAB_CODE, &state, usx_hcodes,
                                                        usx_hcode_lens));
        }
        else
        {
            ...
            if (uni)
            {
                ...
                if (state != USX_DELTA)
                {
                    ...
                    if (uni2)
                    {
                        ...
                        SAFE_APPEND_BITS2(rawolen,
                                          ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA],
                                                           usx_hcode_lens[USX_ALPHA]));
                        SAFE_APPEND_BITS2(
                            rawolen, ol = append_bits(out, olen, ol, usx_vcodes[1],
                    ...
                    else
                    {
                        SAFE_APPEND_BITS2(rawolen, ol = append_switch_code(out, olen, ol, state));
                        SAFE_APPEND_BITS2(rawolen,
                                          ol = append_bits(out, olen, ol, usx_hcodes[USX_DELTA],
            ...
            else
            {
                ...
                for (int bi = l + 1; bi < len; bi++)
                {
                    ...
                    if (readUTF8(in, len, bi, &utf8len))
                        break;
                    if (bi < (len - 4) && c_bi == in[bi - 1] && c_bi == in[bi + 1] &&
                        c_bi == in[bi + 2] && c_bi == in[bi + 3])
                        break;
                    bin_count++;
                }
                // printf("Bin:%d:%d:%x:%d\n", l, (unsigned char) c_in, (unsigned char) c_in,
                // bin_count);
                SAFE_APPEND_BITS2(rawolen, ol = append_nibble_escape(out, olen, ol, state,
                                                                     usx_hcodes, usx_hcode_lens));
...
int unishox2_compress(const char* in, int len, UNISHOX_API_OUT_AND_LEN(char* out, int olen),
                      ...
                      const char* usx_freq_seq[], const char* usx_templates[])
{
    return unishox2_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes,
                                   usx_hcode_lens, usx_freq_seq, usx_templates, NULL);
}

// Main API function. See unishox2.h for documentation
int unishox2_compress_simple(const char* in, int len, char* out)
{
    return unishox2_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1),
...
int readVCodeIdx(const char* in, int len, int* bit_no_p)
{
    if (*bit_no_p < len)
    {
        ...
        do
        {
            ...
            {
                uint8_t vcode =
                    usx_vcode_lookup[usx_vsection_pos[i] +
                ...
                if (h == USX_NUM && v == 0)
                {
                    ...
                    if (idx == 0)
                    {
                        ...
                        for (int j = 0; j < rem; j++)
                        {
                            ...
                            {
                                char nibble_len = (c_t == 'f' || c_t == 'F'
                                                       ? 4
                                                       : (c_t == 'r' ? 3 : (c_t == 't' ? 2 : 1)));
                                const int32_t raw_char =
                                    getNumFromBits(in, len, bit_no, nibble_len);
                                if (raw_char < 0)
                                {
                                    eof = 1;
                                    break;
                    ...
                    else
                    {
                        ...
                        do
                        {
                            ...
                            if (nibble < 0)
                                break;
                            DEC_OUTPUT_CHAR(out, olen, ol++,
                                            getHexChar(nibble, idx < 3 ? USX_NIB_HEX_LOWER
                                                                       : USX_NIB_HEX_UPPER));
                            if ((idx == 2 || idx == 4) &&
                                (nibble_count == 25 || nibble_count == 21 || nibble_count == 17 ||
                                 nibble_count == 13))
                                DEC_OUTPUT_CHAR(out, olen, ol++, '-');
                            bit_no += 4;
                        } while (--nibble_count);
...
int unishox2_decompress(const char* in, int len, UNISHOX_API_OUT_AND_LEN(char* out, int olen),
                        ...
                        const char* usx_freq_seq[], const char* usx_templates[])
{
    return unishox2_decompress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes,
                                     usx_hcode_lens, usx_freq_seq, usx_templates, NULL);
}
